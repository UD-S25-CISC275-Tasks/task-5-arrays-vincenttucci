...
export function makeMath(addends: number[]): string {
    ...
    let sum = addends.reduce((a, b) => a + b);
...
export function injectPositive(values: number[]): number[] {
    const i = values.findIndex((x) => x < 0);
    let sum = 0;

    if (i !== -1) {
        sum = values
            .slice(0, i)
            .reduce((accumulation, X) => accumulation + X, 0);
        return [...values.slice(0, i + 1), sum, ...values.slice(i + 1)];
    } else {
        sum = values.reduce((accumulation, X) => accumulation + X, 0);
        return [...values, sum];
    }
}
